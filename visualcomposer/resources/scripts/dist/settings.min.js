...
(function($) {
    ...
    $(function() {
        ...
        $form.submit(function(e) {
            ...
            $("#vc_wp-spinner").show();
            data.action = $("#settings-save-roles-btn").val();
            data.vc_nonce_field = $("#vc_nonce_field").val();
            data.vc_roles = {};
            $("[data-vc-role]").each(function() {
                var $this = $(this), role = $this.data("vcRole"), roleData = {};
                $this.find("select").each(function() {
                    var $this = $(this), part = $this.data("vcPart");
    ...
    $('[data-vc-ui-element="panel-tab-control"]').click(function(e) {
        var $control = $(this), $fieldset = $control.parents("fieldset").first(), filter = ".vc_wp-form-table", filterValue;
        e.preventDefault();
    ...
    $('[data-vc-roles="table-checkbox"]').change(function() {
        var $this = $(this);
        if ($this.is(":checked")) {
            $this.parents("tr:first").find('[data-vc-name!="' + $this.attr("data-vc-name") + '"]:checked').prop("checked", false);
        } else {
            $this.parents('[data-vc-roles="table"]').first().find('[data-vc-roles-select-all-checkbox="' + $this.data("vcCap") + '"]').prop("checked", false);
        }
    });
    $("[data-vc-roles-select-all-checkbox]").change(function() {
        ...
        $parent.find('[data-vc-cap="' + value + '"]:visible').prop("checked", checked);
        $relatedControl.prop("checked", checked);
        if (checked) {
            _.defer(function() {
                $parent.find("[data-vc-roles-select-all-checkbox!=" + value + "]:not([data-vc-cap])").prop("checked", false);
                $parent.find('[data-vc-cap!="' + value + '"]:not([data-vc-roles-select-all-checkbox]):visible').prop("checked", false);
            });
        }
    });
    $('[data-vc-role-related-part].vc_visible [data-vc-roles="table"]').each(function() {
        ...
        $table.find("thead [data-vc-roles-select-all-checkbox]").each(function() {
            var $this = $(this), value = $this.data("vcRolesSelectAllCheckbox");
        ...
        vc.less.generateLessFormData = function(formData, variablesData) {
            ...
            if (!_.isEmpty(variablesData)) {
                _.each(variablesData, function(value, key) {
                    ...
                    if (_.isString(value)) {
                        object = _.first(_.where(formData, {
                            name: value
                        }));
...
(function($) {
    ...
    var message_timer, template_options = {
        evaluate: /<#([\s\S]+?)#>/g,
        interpolate: /\{\{\{([\s\S]+?)\}\}\}/g,
        escape: /\{\{([^\}]+?)\}\}(?!\})/g
    ...
    var request_url = window.ajaxurl + "?vc_action=automapper", sync_callback = function(method, model, options) {
        var data;
        if ("create" === method) {
    ...
    var ComplexShortcodeView = Backbone.View.extend({
        ...
        edit: function(e) {
            ...
            if (this.edit_view) {
                ...
                this.edit_view = new EditFormInnerView({
                    model: this.model
                }).render();
            }
        },
    ...
    var EditFormView = FormView.extend({
        ...
        setParamFieldActive: function(e) {
            var $control = $(e.currentTarget), index = $control.parents(".vc_param:first").index();
            this.active_preview && $(this.active_preview).removeClass("vc_active");
        ...
        getPreview: function(data) {
            var params = data.params, content = false, params_to_string = {};
        ...
        validate: function(attrs) {
            ...
            _.each(attrs.params, function(param, index) {
                ...
                if (_.isBoolean(added_param_names[param.param_name]) && true == added_param_names[param.param_name]) {
                    ...
                    $field_el.closest(".vc_param-field").addClass("form-invalid");
                    if (!result) {
                        result = window.i18nLocaleVcAutomapper.error_param_already_exists.replace(/\%s/, param.param_name);
                    }
                }
                added_param_names[param.param_name] = true;
                _.each(fields_required, function(field) {
                    if ("hidden" !== param.type && "" === param[field] || "hidden" === param.type && "heading" !== field && "" === param[field]) {
                        $("#vc_atm-params-list [name=" + field + "]:eq(" + index + ")").addClass("vc_error").closest(".vc_param-field").addClass("form-invalid");
                        if (!result) {
                            result = window.i18nLocaleVcAutomapper.error_enter_required_fields;
                        }
                    } else if ("param_name" === field && !param[field].match(/^[a-z0-9_]+$/g)) {
                        $field_el.addClass("vc_error").closest(".vc_param-field").addClass("form-invalid");
                        if (!result) {
    ...
    var EditFormInnerView = EditFormView.extend({
        ...
        render: function() {
            var params, parent = this.model.view;
            params = this.model.get("params");
            EditFormView.__super__.render.call(this);
            this.$el.html(_.template(this.template_html, _.extend({
                shortcode_preview: this.getPreview(this.model.toJSON())
            }, this.model.toJSON()), template_options));
...
jQuery(document).ready(function($) {
    ...
    $("#vc_settings-updater-button").click(function(e) {
        var $this = $(this), action = $this.data("vcAction"), $parent = $this.parent();
    ...
    $("#vc_settings-color").submit(function(e) {
        ...
        if ($designCheckBox.prop("checked") && "restore_color" !== $("#vc_settings-color-action").val()) {
            ...
            $spinner = $('<span class="vc_settings-spinner vcv-ui-wp-spinner"></span>');
                ...
                vc.less.build({
                    ...
                    if (!_.isUndefined(output) && !_.isUndefined(output.css)) {
                        ...
                        $.ajax({
                            ...
                            success: function() {
                                showMessageMore(window.i18nLocaleSettings.saved, "updated", 5e3, true).insertBefore($submitButton.parent()).fadeIn(500);
                                $submitButton.val(window.i18nLocaleSettings.save);
                                lessBuilding = false;
                                $spinner.remove();
                            },
                            error: function() {
                                showMessageMore(window.i18nLocaleSettings.form_save_error, "error", undefined, true).insertBefore($submitButton.parent()).fadeIn(500);
                                $submitButton.val(window.i18nLocaleSettings.save);
                                lessBuilding = false;
                                $spinner.remove();
                            }
                        ...
                        showMessageMore(window.i18nLocaleSettings.save_error + ". " + error, "error", undefined, true).insertBefore($submitButton.parent()).fadeIn(500);
                        $submitButton.val(window.i18nLocaleSettings.save);
                        lessBuilding = false;
                        $spinner.remove();
                    }
