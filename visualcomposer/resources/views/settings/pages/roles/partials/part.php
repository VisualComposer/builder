        ...
        <fieldset>
            ...
            <select name="<?= esc_attr($paramsPrefix . '[_state]') ?>"
                ...
                <?php if (!empty($capabilities)) : ?>
                    ...
                    data-vc-role-part="<?= esc_attr($part . '-' . $role) ?>"
                <?php endif ?>
                ...
                <?php foreach ($options as $option) : ?>
                    <option
                        ...
                        <?= $controller->getState() === $option[0] ? ' selected' : '' ?>><?= esc_html(
                            $option[1]
                        ) ?></option>
                <?php endforeach ?>
            </select>
...
<?php if (!empty($capabilities)) : ?>
    <?php if (isset($useTable) && true === $useTable) : ?>
        ...
        <tr data-vc-role-related-part="<?= esc_attr($part . '-' . $role) ?>"
            ...
            class="vc_role-custom-settings<?= !isset($customValue)
                                ...
                                <th class="column-date">
                                    <label>
                                        <input type="checkbox" name="all"
                                            ...
                                            data-vc-roles-select-all-checkbox="<?= esc_attr($type[0]) ?>"><?= esc_html(
                                            $type[1]
                                        ) ?>
                                    </label>
                                </th>
                            ...
                            <?php if (!isset($ignoreCapabilities) || !in_array($cap['base'], $ignoreCapabilities)) : ?>
                                ...
                                <tr data-vc-capability="<?= esc_attr($cap['base']) ?>"
                                    ...
                                    <td title="<?= esc_attr($cap['base']) ?>">
                                        ...
                                        <div>
                                            <?= esc_html($cap['name']) ?>
                                            <?= !empty($cap['description']) ? '<span class="vc_element-description">'
    ...
    <?php else : ?>
        <tr data-vc-role-related-part="<?= esc_attr($part . '-' . $role) ?>"
            data-vc-role-part-state="<?= esc_attr(isset($customValue) ? $customValue : '*') ?>"
            class="vc_role-custom-settings<?= !isset($customValue) || $controller->getState() === $customValue
                ? ' vc_visible' : '' ?>">
            <th scope="row"></th>
            <td>
                <fieldset>
                                ...
                                <label>
                                    <input type="checkbox"
                                        ...
                                        if ('administrator' === $role && 'settings' === $part
                                            ...
                                            data-vc-roles="checkbox"<?= $controller->can($cap[0], false)->get()
                                                ? ' checked' : '' ?>
                                            <?php
                                        }
                                        ?>>
