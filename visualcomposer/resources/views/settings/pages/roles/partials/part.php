...
<tr>
    <th scope="row">
        <?php echo esc_html($mainLabel) ?>
    </th>
    <td>
        <fieldset>
            <legend class="screen-reader-text">
                <span><?php esc_html($mainLabel) ?></span></legend>
            <select name="<?php echo esc_attr($paramsPrefix . '[_state]') ?>"
                ...
                <?php foreach ($options as $option) : ?>
                    <option
                        ...
                        <?php echo $controller->getState() === $option[0] ? ' selected' : '' ?>><?php echo esc_html(
                            $option[1]
                        ) ?></option>
                <?php endforeach ?>
            </select>
...
<?php if (!empty($capabilities)) : ?>
    <?php if (isset($useTable) && true === $useTable) : ?>
        ...
        <tr data-vc-role-related-part="<?php echo esc_attr($part . '-' . $role) ?>"
            data-vc-role-part-state="<?php echo esc_attr(isset($customValue) ? $customValue : '*') ?>"
            class="vc_role-custom-settings<?php echo !isset($customValue)
            || (string)$controller->getState() === (string)$customValue ? ' vc_visible' : '' ?>">
            <th scope="row"></th>
            <td>
                <fieldset>
                    <legend class="screen-reader-text">
                        <span><?php echo esc_html($customLabel) ?></span>
                    </legend>
                    <?php if (isset($categories) && !empty($categories)) : ?>
                        <?php vc_include_template(
                            'editors/partials/add_element_tabs.tpl.php',
                            [
                    ...
                    <table class="vc_general vc_wp-form-table fixed" data-vc-roles="table">
                        <thead>
                        <tr>
                            <th><?php echo esc_html($itemHeaderName) ?></th>
                        ...
                        <tfoot<?php echo isset($globalSet) ? ' style="display: none;"' : '' ?>>
                        <tr>
                            <th><?php echo esc_html($itemHeaderName) ?></th>
                            <?php foreach ($capTypes as $type) : ?>
                                <th class="column-date">
                        ...
                        <tbody<?php echo isset($globalSet) ? ' style="display: none;"' : '' ?>>
                        <?php foreach ($capabilities as $cap) : ?>
                            <?php if (!isset($ignoreCapabilities) || !in_array($cap['base'], $ignoreCapabilities)) : ?>
                                ...
                                <tr data-vc-capability="<?php echo esc_attr($cap['base']) ?>"
                                    class="<?php echo esc_attr(trim($categoryCssClasses)) ?>">
    ...
    <?php else : ?>
        <tr data-vc-role-related-part="<?php echo esc_attr($part . '-' . $role) ?>"
            data-vc-role-part-state="<?php echo esc_attr(isset($customValue) ? $customValue : '*') ?>"
            class="vc_role-custom-settings<?php echo !isset($customValue) || $controller->getState() === $customValue
                ? ' vc_visible' : '' ?>">
            <th scope="row"></th>
            <td>
                <fieldset>
                    ...
                    <div class="vc_wp-form-row">
                        <?php foreach ($capabilities as $cap) : ?>
                            <div class="vc_wp-form-col vc_wp-form-checkbox">
                                <label>
                                    <input type="checkbox"
                                        name="<?php echo esc_attr($paramsPrefix . '[' . $cap[0] . ']') ?>"
                                        value="1"
                                        class="vc_roles-settings-checkbox"
                                        <?php

                                        if ('administrator' === $role && 'settings' === $part
                                            && ('vc-v-roles-tab' === $cap[0] || 'vc-v-license-tab' === $cap[0])
                                        ) {
                                            ...
                                            data-vc-roles="checkbox"<?php echo $controller->can($cap[0], false)->get()
                                                ? ' checked' : '' ?>
                                            <?php
                                        }
                                        ?>>
