...
class TestUpdates extends WP_UnitTestCase
{
    public function testCheckCreateToken()
    {
        ...
        $token = true;
        add_filter('pre_http_request', function ($response) use (&$token) {
            $args = func_get_args();
            if ($token) {
                $token = false;
                return '';
    ...
    public function testCheckCreateTokenSuccess()
    {
        $loggerHelper = vchelper('Logger');
        $x = true;
        add_filter('pre_http_request', function ($response) use (&$x) {
            $args = func_get_args();
            if ($x) {
                $x = false;
    ...
    public function testCheckVersionHubUpdate()
    {
        ...
        add_filter('pre_http_request', function ($response, $body, $url) use (&$token, &$actions) {
            ...
            if ($actions) {
                ...
                $this->assertEquals(VCV_HUB_URL . '/download/json/lite?plugin=' . VCV_VERSION . '&token=temp2token', $url);
                return ['body' => '{"actions":[]}'];
            }
            return $response;
        }, 10, 100);
