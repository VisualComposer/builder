...
class UpdatePredefinedTemplatesBundleBuilder {
  ...
  copyFiles () {
    ...
    const jsonFilename = 'templates.bundle.json'
    const prebuildDir = join(__dirname, '..', 'sources', 'bundlePrebuild', 'templates')
    const prebuildJSON = join(__dirname, '..', 'sources', 'bundlePrebuild', jsonFilename)
    const bundleDir = join(this.dir, this.bundleDir)
    return Promise.all(
      [ fs.copy(prebuildDir, join(bundleDir, 'templates')),
        fs.copy(prebuildJSON, join(bundleDir, 'bundle.json')) ])
  }

  createZipArchiveAsync (version) {
    const bundleZipPath = path.join(this.dir, `templates-${version || 0.1}.bundle.zip`)
...
module.exports = UpdatePredefinedTemplatesBundleBuilder