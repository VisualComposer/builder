import vcCake from 'vc-cake'

vcCake.env('FEATURE_WEBPACK', false)
vcCake.env('debug', false) // Set false on real production deploy
vcCake.env('FIX_DND_FOR_TABS', true)
vcCake.env('CONTAINER_DIVIDER', true)
vcCake.env('MOBILE_DETECT', true)
vcCake.env('HUB_TEASER', true)
vcCake.env('FEATURE_ASSETS_FILTER', true)
vcCake.env('CONTAINER_DIVIDER_EMBED_VIDEO', true)
vcCake.env('DND_SMART_LINE_TRANSITION', true)
vcCake.env('DND_TRASH_BIN', true)
vcCake.env('DND_DISABLE_DROP_IN_CLOSED_TABS', true)
vcCake.env('ELEMENT_CONTROLS_DELAY', true)
vcCake.env('CSS_LOADING', true)
vcCake.env('EXISTING_ELEMENT_ATTR_FIX', true)
vcCake.env('PAGE_TEMPLATES_FE', true)
vcCake.env('DIVIDER_YOUTUBE', true)
vcCake.env('CUSTOM_JS', true)
vcCake.env('NAVBAR_SINGLE_CONTENT', true)
vcCake.env('DIVIDER_VIMEO', true)
vcCake.env('HUB_TEASER_ELEMENT_DOWNLOAD', true)
vcCake.env('UI_NOTIFICATIONS', true)
vcCake.env('DIVIDER_SHAPE_UPDATE', true)
vcCake.env('PAGE_TITLE_FE', true)
vcCake.env('ATTRIBUTE_LIBS', true)
vcCake.env('IFRAME_RELOAD', true)
vcCake.env('TEASER_DROPDOWN_UPDATE', true)
vcCake.env('FIX_UNREGISTERED_ELEMENT', true)
vcCake.env('FF_QUANTUM_DO', true)
vcCake.env('FE_SHORTCODES_SCRIPTS', true)
vcCake.env('FE_CONTENTEDITABLE_REFS', true)
vcCake.env('FE_LOGO_LINK_PREMIUM', true)
vcCake.env('GOOGLE_FONTS', true)
vcCake.env('FE_TOGGLE_ELEMENT', true)
vcCake.env('PARALLAX_MOUSEMOVE', true)
vcCake.env('HUB_REDESIGN', true)
vcCake.env('DISABLE_COLUMN_STACKING', true)
vcCake.env('CLASSIC_TABS', true)
vcCake.env('IMAGE_FILTERS', true)
vcCake.env('DND_FIX_TREEVIEW_CLOSED', true)
vcCake.env('CLASSIC_ACCORDION', true)
vcCake.env('REFACTOR_ELEMENT_ACCESS_POINT', true)
vcCake.env('COLORPICKER_LAST_USED', true)
vcCake.env('TF_SHOW_PLUGIN_UPDATE', true)
vcCake.env('TF_HEARTBEAT_HAS_CLASS_ERROR', false)
vcCake.env('CSS_GRID', false)
vcCake.env('SAVE_API', true)
// DO NOT MODIFY THIS!!!
vcCake.env('THEME_LAYOUTS', false) // SEE the devAddons/themeEditor/themeEditor/src/*.js files
vcCake.env('THEME_EDITOR', false)
// END OF DO NOT MODIFY
vcCake.env('EXCLUDE_ATTRIBUTE', true)
vcCake.env('SAVE_ZIP', true)
vcCake.env('HFS_START_PAGE', true)
vcCake.env('HIDE_ATTRIBUTES_DEPENDING_ON_EDITOR', true)
vcCake.env('REPLACE_ELEMENTS', true)
vcCake.env('HUB_CONTROLS', true)
vcCake.env('REMOVE_SETTINGS_SAVE_BUTTON', true)
vcCake.env('PAGE_TEMPLATE_LAYOUTS', true)
vcCake.env('TF_FIX_EMPTY_ELEMENT_SECTION', true)
vcCake.env('PAGE_TEMPLATE_PREVIEW', true)
vcCake.env('TF_TEMPLATES_DROPDOWN_UPDATE', true)
vcCake.env('TF_SETTINGS_THEME_ICONS', true)
vcCake.env('TF_DISABLE_BACKEND', true)
vcCake.env('TF_RENDER_PERFORMANCE', true)
vcCake.env('CACHE_HOVER_CONTROLS', true)
vcCake.env('TF_FREE_VERSION_DOWNLOAD', false)
vcCake.env('SINGLE_IMAGE_REFACTOR', true)
vcCake.env('TF_ELEMENT_CRUD_PERFORMANCE', false)
vcCake.env('TF_PARAM_GROUP_CSS_MIXINS', false)
vcCake.env('TEMPLATE_PANEL_PERF', true)
vcCake.env('VCV_HUB_ADDON_TEASER', true)
vcCake.env('FIX_SHORTCODE_BLOCKS_TEXT_BLOCK_EDIT', true)
vcCake.env('TEMPLATE_SAVE_FULL_DATA', true)
vcCake.env('FT_WAIT_FOR_FINAL_RENDER', true)
vcCake.env('FT_COPY_PASTE_FOR_COLUMN', false)
vcCake.env('FT_COPY_PASTE_FOR_ROW', false)
vcCake.env('FT_CSS_CACHE', false)
