...
Builder.prototype.checkControls = function(center) {
  ...
  Object.keys(this.atolls).forEach(function(key) {
    var atoll = this.atolls[key];
    // if (atoll.el !== this.dragingElement) {
    nearestControl = atoll.hasNearestControl(nearestControl, center);
    // }
  }, this);
  if (this.nearestControl.control) {
    this.nearestControl.control.setSize(0);
  }
  this.nearestControl = nearestControl;
  if (this.nearestControl.control) {
    this.nearestControl.control.setSize(4);
  }
};
