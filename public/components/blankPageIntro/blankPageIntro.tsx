...
const BlankPageIntro: React.FC<Props> = ({ unmountBlankPage }) => {
  ...
  const updateLayout = useCallback((data: { value: string }) => {
    if (!env('VCV_JS_THEME_EDITOR')) {
      ...
      const isHeaderSet = dataManager.get('headerTemplates') && dataManager.get('headerTemplates').current
      const lastLoadedHeaderTemplate = window.vcvLastLoadedHeaderTemplate || isHeaderSet
      const lastSavedHeaderTemplate = settingsStorage.state('headerTemplate').get()

      const isSidebarSet = dataManager.get('sidebarTemplates') && dataManager.get('sidebarTemplates').current
      const lastLoadedSidebarTemplate = window.vcvLastLoadedSidebarTemplate || isSidebarSet
      const lastSavedSidebarTemplate = settingsStorage.state('sidebarTemplate').get()

      const isSFooterSet = dataManager.get('footerTemplates') && dataManager.get('footerTemplates').current
      const lastLoadedFooterTemplate = window.vcvLastLoadedFooterTemplate || isSFooterSet
