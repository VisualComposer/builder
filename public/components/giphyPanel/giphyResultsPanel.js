...
export default class GiphyResultsPanel extends React.Component {
  static propTypes = {
  ...
  getImagesFromServer (searchValue, page, action = 'search') {
    ...
    const unsplashUrl = `${vcvApiUrl}/api/giphy/${action}`
  ...
  handleClickDownloadImage (e) {
    ...
    if (imageId && size) {
      ...
      dataProcessor.appServerRequest({
        'vcv-action': 'hub:giphy:download:adminNonce',
        ...
        try {
          ...
          if (jsonData.status) {
            ...
            errorMessage = errorMessage || `${GiphyResultsPanel.localizations.noAccessCheckLicence} #10087` || 'No access, please check your license! #10087'
            notificationsStorage.trigger('add', {
              position: 'bottom',
              transparent: true,
              rounded: true,
          ...
          const exceptionErrorMessage = `${GiphyResultsPanel.localizations.coundNotParseData} #10086` || 'Could not parse data from server! #10086'
          notificationsStorage.trigger('add', {
            position: 'bottom',
            transparent: true,
            rounded: true,
              ...
              <div className='vcv-stock-image-download-container'>
                <div className='vcv-stock-image-download-options'>
                  <button
                    ...
                    data-img-size='small'
                  >
                    {GiphyResultsPanel.localizations.small || 'Small'}
                  </button>
                  <button
                    className='vcv-stock-image-download-button'
                    onClick={this.handleClickDownloadImage}
                    data-img-size='regular'
                  >
                    {'Regular'}
                  </button>
                  <button
  ...
  getNoResultsElement () {
    const nothingFoundText = GiphyResultsPanel.localizations ? GiphyResultsPanel.localizations.nothingFound : 'Nothing found'
