...
+(function ($) {
  ...
  var Tabs, old, clickHandler, changeHandler, setupTabsProperty
  var settings = {}
  ...
  function Plugin (action, options) {
    ...
    return this.each(function () {
      ...
      $this = $(this)
      data = $this.data(settings.tabsDataSelector)
      if (!data) {
  ...
  changeHandler = function (e) {
    var caller
    caller = $(e.target).data(settings.accordionContainer)
    ...
    if (typeof (caller.getRelatedTab) === 'undefined') {
      ...
      caller.getRelatedTab = function () {
        ...
        findTargets = function () {
          var $targets
          $targets = caller.getContainer().find(settings.dataAttr).filter(function () {
            var $this, tab
            $this = $(this)
  ...
  window.vcvTabsInit = function (options) {
    settings = options
    setupTabsProperty()
    setActiveTab()
    $(document).on(settings.clickEventSelector, settings.dataAttr, clickHandler)
    $(document).on(settings.showEventSelector, changeHandler)
  }
